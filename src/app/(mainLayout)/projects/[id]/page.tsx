...
const ProjectDetails: React.FC<PageProps> = ({ params }) => {
  ...
  useEffect(() => {
    const fetchProjectData = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_API_URL}/project/${id}`, {
          cache: "no-store",
        });
      ...
      <Container className="my-20">
        <div className="lg:flex gap-10">
          ...
          <div className="w-full mt-5 lg:mt-0">
            {activeTab === "tab1" && <Overview projectData={projectData.data} />}
            ...
            {activeTab === "tab5" && <VirtualTour projectData={projectData.data} />}
